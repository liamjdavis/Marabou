...
private:
    ...
    Key &mutable_key()
    {
        return const_cast<Key &>( d_value.first );
    }
    // See documentation of value_type for why this is needed.
    Data &mutable_data()
    {
        return const_cast<Data &>( d_value.second );
    }
    ...
    void restore( ContextObj *data ) override
    {
        ...
        if ( d_map != NULL )
        {
            if ( p->d_map == NULL )
            {
                ...
                {
                    Debug( "gc" ) << "remove nonfirst-elem " << this << " from map " << d_map
                                  << std::endl;
                }
 ...
 */
template <class Key, class Data, class HashFcn> class CDHashMap : public ContextObj
{
...
public:
    ...
    ~CDHashMap()
    {
        Debug( "gc" ) << "cdhashmap" << this << " disappearing, destroying..." << std::endl;
        destroy();
        Debug( "gc" ) << "cdhashmap" << this << " disappearing, done destroying" << std::endl;
        clear();
    }
    ...
    void insertAtContextLevelZero( const Key &k, const Data &d )
    {
        ...
        Element *obj = new ( true ) Element( d_context, this, k, d, true /* atLevelZero */ );
        d_map.insert( std::make_pair( k, obj ) );
    }
