    ...
    else if ( !phaseFixed() )
    {
        ...
        if ( isActive() )
        {
            ...
            else if ( bound < 0 && variable == _f )
            {
                if ( proofs )
                    _boundManager->addLemmaExplanationAndTightenBound(
                        _f, 0, Tightening::LB, { variable }, Tightening::LB, *this, false, 0 );
                else
...
PiecewiseLinearCaseSplit ReluConstraint::getInactiveSplit() const
{
    ...
    if ( !_cdclVars.empty() )
    {
        ...
        inactivePhase.addCdclLiteral( -(int)_cdclVars.front() );
    }
...
PiecewiseLinearCaseSplit ReluConstraint::getActiveSplit() const
{
    ...
    {
        ASSERT( _cdclVars.size() == 1 )
        activePhase.addCdclLiteral( (int)_cdclVars.front() );
    }
    ...
    Map<unsigned int, PiecewiseLinearConstraint *> &cadicalVarToPlc )
{
    ...
    ASSERT( _cdclVars.size() == 1 && _cdclVars.front() == idx );
